import { stat } from "fs";
import { type MapData, BLANK, WALL, BOX_L, BOX_R, PLAYER, UP, DOWN, LEFT, RIGHT, parse_map, parse_moves } from "./util.mjs";

type MoveType = (typeof UP | typeof DOWN | typeof RIGHT | typeof LEFT);

const draw_background = (ctx: CanvasRenderingContext2D) => {
    ctx.fillStyle = "rgba(140, 140, 200, 0.5)";
    ctx.fillRect(0, 0, 800, 800);
};

const render = (ctx: CanvasRenderingContext2D, state: MapData) => {
...
const check_move_box_updown = (state: MapData, box_coord: [number, number], next_move: MoveType): boolean => {
    let [bx, by] = box_coord;
    if (state.map[bx][by] === BOX_R) bx -= 1;
    switch (next_move) {
        case UP: {
            const up_tile_left = state.map[bx][by - 1];
            const up_tile_right = state.map[bx + 1][by - 1];
            if (up_tile_left === WALL || up_tile_right === WALL) return false;
            if (up_tile_left === BOX_L || up_tile_left === BOX_R) {
                if (!check_move_box_updown(state, [bx, by - 1], next_move)) return false;
            }
            if (up_tile_right === BOX_L) {
                if (!check_move_box_updown(state, [bx + 1, by - 1], next_move)) return false;
            }
            return true;
        }
        case DOWN: {
            ...
            if (down_tile_left === WALL || down_tile_right === WALL) return false;
...
const move_box = (state: MapData, box_coord: [number, number], next_move: MoveType) => {
    let [bx, by] = box_coord;
    switch (next_move) {
        case UP: {
            if (state.map[bx][by] === BOX_R) bx -= 1;
            const up_tile_left = state.map[bx][by - 1];
            const up_tile_right = state.map[bx + 1][by - 1];
            if (up_tile_left === BOX_L) move_box(state, [bx, by - 1], next_move);
            if (up_tile_left === BOX_R) move_box(state, [bx, by - 1], next_move);
            if (up_tile_right === BOX_L) move_box(state, [bx + 1, by - 1], next_move);
        ...
        case DOWN: {
            ...
            const down_tile_left = state.map[bx][by + 1];
            const down_tile_right = state.map[bx + 1][by + 1];
            if (down_tile_left === BOX_L) move_box(state, [bx, by + 1], next_move);
            if (down_tile_left === BOX_R) move_box(state, [bx, by + 1], next_move);
            if (down_tile_right === BOX_L) move_box(state, [bx + 1, by + 1], next_move);
...
const move_player = (state: MapData, next_move: MoveType) => {
    ...
    switch (next_move) {
        case UP: {
            ...
            if (!check_move_box_updown(state, [px, py - 1], next_move)) return;
...
const calc_score = (state: MapData) => {
    let score = 0;
    for (let i = 0; i < state.width; i++) {
...
(async () => {
    ...
    if (!game_canvas) throw new Error("No game_canvas found");
    const ctx = game_canvas.getContext("2d");
    if (!ctx) throw new Error("Canvas 2d ctx not supported");
    ...
    let moves = [] as MoveType[];

    const moves_textfield = document.createElement("textarea");
    moves_textfield.oninput = (evt) => {
        // @ts-ignore
        const _txt = evt.target.value as string;
        moves = _txt.split("").filter(val => (val === UP || val === DOWN || val === LEFT || val === RIGHT)) as MoveType[];
    ...
    [UP, DOWN, LEFT, RIGHT].forEach(dir => {
        ...
        btn.onclick = (evt) => {
            evt.preventDefault();
            moves.push(dir as typeof moves[number]);
    ...
    load_moves_btn.onclick = async (evt) => {
        ...
        moves = move_string.split("").filter(val => (val === UP || val === DOWN || val === LEFT || val === RIGHT)) as MoveType[];
        moves_textfield.value = moves.join("");
    };
